...
export default function LoginModal(props: any){
    ...
    async function recoverPassword() {
        if(formData.email.trim() === ""){
            ...
            const { data, error } = await supabase.auth.api.resetPasswordForEmail(formData.email, {
                redirectTo: `${process.env.NEXT_PUBLIC_SITE_URL}/profile/passwordRecovery`,
            })
    ...
    async function registerWithEmail() {
        ...
        const { data : checkUsernameData, error } : any = await supabase.from("profiles").select("*").eq("username", formRegisterData.registerUsername)
        const { data : checkEmailData, error: emailError } : any = await supabase.from("profiles").select("*").eq("email", formRegisterData.registerEmail)

        if (error || emailError) {
            setShowAlert(true)
            setAlertMessage(error.message || emailError.message)
            return true;
        } else { 

            if (checkUsernameData?.length > 0 || checkEmailData?.length > 0) {
                setShowAlert(true)
                setAlertMessage("Username or E-Mail already exists")
                return true;
            }else{
                ...
                if(error2 || error){
                    setShowAlert(true)
                    setAlertMessage(error2?.message || error?.message || "An unknown error occurred")
                    return true;
                }
